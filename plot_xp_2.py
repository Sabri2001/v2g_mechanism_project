...
def compute_savings(data):
    ...
    for (battery_factor, alpha_factor), costs in data.items():
        ...
        bw_reduction = 100 * (1 - battery_factor)
        result[alpha_factor].append((bw_reduction, mean_saving, std_saving))
...
def plot_savings(savings_by_alpha):
    ...
    for alpha, points in savings_by_alpha.items():
        ...
        plt.errorbar(x, y, yerr=yerr, label=f"Inflexibility coefficient = {alpha}", marker='o', capsize=4)

    plt.xlabel("Battery Wear Cost Reduction (%)")
    plt.ylabel("Cost Savings (%)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("outputs/tsg/xp_2/summary_cost_savings_vs_battery_wear.png")
