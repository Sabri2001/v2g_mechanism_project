...
class UnidirectionalCoordinatedSchedulingExperiment(BaseExperiment):
    ...
    def _solve_local_subproblem(self, evs, ev_idx, state, old_state, market_prices, start_time, end_time):
        ...
        for t in range(T):
            ...
            operator_cost += beta_wear * u[t] * eff
        ...
        for t in range(T):
            ...
            model.addConstr(
                w[t] >= mu[t] + beta * (u[t] + sum_other - self.config["evcs_power_limit"]),
                name=f"wPos_{t}"
            )
        ...
        # 5) SoC threshold and charging constraints
        for t in range(T):
    ...
    def _global_step(self, state, evcs_power_limit):
        ...
        new_mu = np.maximum(0, mu + beta * (sum_u - evcs_power_limit))
        state["mu"] = new_mu
