...
def run_nash_check(config, base_output_dir):
    ...
    # under keys "true_disconnection_time" and "true_disconnection_time_preference_coefficient".
    ...
    for ev in config["evs"]:
        if ev["id"] == ev_nash_id:
            ...
            true_alpha_global = ev.get("true_disconnection_time_preference_coefficient",
                                       ev["disconnection_time_preference_coefficient"])
            break
        ...
        for j, alpha_bid in enumerate(alpha_values):
            ...
            for ev in config_nash["evs"]:
                if ev["id"] == ev_nash_id:
                    ...
                    ev["disconnection_time_preference_coefficient"] = alpha_bid
                    ev_found = True
                    break
            if not ev_found:
