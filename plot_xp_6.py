...
def collect_data(base_dir="outputs/tsg/xp_6"):
    ...
    for path in glob(os.path.join(base_dir, "cost_savings_limit_*_af_*")):
        ...
        for exp_type in ["centralized", "uncoordinated"]:
...
def compute_savings(data):
    ...
    for (limit_factor, alpha_factor), costs in data.items():
        ...
        uncoord = costs["uncoordinated"]
        if len(coord) != len(uncoord):
            continue  # Skip mismatched runs

        savings = [(u - c) / u * 100 for c, u in zip(coord, uncoord)]
        mean_saving = np.mean(savings)
        std_saving = np.std(savings)
        alpha_reduction = 100 * (1 - alpha_factor)
        result[limit_factor].append((alpha_reduction, mean_saving, std_saving))
...
def plot_savings(savings_by_limit):
    ...
    for limit, points in sorted(savings_by_limit.items()):
        ...
        plt.errorbar(x, y, yerr=yerr, label=f"EVCS power limit = {limit}", marker='o', capsize=4)

    plt.xlabel("Inflexibility Reduction (%)")
